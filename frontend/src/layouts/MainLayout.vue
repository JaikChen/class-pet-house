<template>
  <div class="min-h-screen text-gray-800 pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-4 overflow-x-hidden" :style="{ backgroundColor: 'var(--theme-bg, #fef5b5)' }">
    ...
    <div v-if="classStore.groups.length" class="max-w-[1600px] 2xl:max-w-[1800px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 mb-4 relative z-40">
      <div class="bg-white/60 backdrop-blur-md border border-white/80 rounded-full px-1.5 py-1.5 flex gap-1.5 overflow-x-auto shadow-sm w-full sm:w-auto custom-scrollbar-hide">
    ...
    <div v-if="batchMode" class="fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom))] md:bottom-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] border-2 border-white rounded-2xl px-4 sm:px-6 py-4 flex flex-col md:flex-row items-center gap-4 md:gap-6 z-50 w-[calc(100%-1rem)] sm:w-[90%] md:w-auto max-w-sm md:max-w-none">
      <span class="font-bold text-gray-600 whitespace-nowrap">已选 <span class="text-accent text-xl mx-1">{{ selectedIds.length }}</span> 人</span>
      <div class="flex gap-3 w-full md:w-auto">
        <button @click="toggleSelectAll" class="btn-toy flex-1 md:flex-none px-5 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-bold text-gray-600 whitespace-nowrap">
          {{ isAllSelected ? '取消全选' : '全选同学们' }}
        </button>
        <button @click="showBatchScoreModal = true" :disabled="!selectedIds.length"
          class="btn-toy flex-1 md:flex-none px-6 py-2.5 bg-accent text-white rounded-xl text-sm font-bold disabled:opacity-50 shadow-[0_4px_0_var(--theme-shadow-hard)] flex items-center justify-center gap-2 whitespace-nowrap">
  ...
  <div class="md:hidden fixed bottom-0 left-0 right-0 z-[999] bg-white shadow-[0_-5px_20px_rgba(0,0,0,0.05)] border-t border-slate-100 pb-[env(safe-area-inset-bottom)]">
    <nav class="max-w-5xl mx-auto flex items-center justify-around px-2 py-2 relative">
...
const { setTheme } = useTheme() // 确保依赖注入，即使模板没有显式调用
...
function updateTopPanelHeight() {
  if (topPanelRef.value) {
    topPanelHeight.value = Math.ceil(topPanelRef.value.getBoundingClientRect().height)
  }
}
...
const groupStats = computed(() => {
  ...
  const students = classStore.students || []
  ...
  const ungroupedCount = counts['ungrouped']
...
onMounted(async () => {
  ...
  try {
    ...
    if (classStore.currentClass) {
      // 此时 fetchStudents, fetchGroups 等都已加上并发锁，可以放心调用
      await Promise.all([
